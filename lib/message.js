 ...
 * @static
 */
Message._readClientId = function (ctx) {
    var id = ctx.buf.slice(ctx.pos, ctx.pos + 20).toString('hex');
    ctx.pos += 20;
 ...
 * @static
 */
Message._writeClientId = function (ctx, idHex) {
    if (ctx.buf.length < ctx.pos + 24) {
        throw new Error("Insufficient buffer");
    }

    // Append attribute header.
    ctx.buf[ctx.pos++] = Const.AttrTypes.clientId >> 8;
    ctx.buf[ctx.pos++] = Const.AttrTypes.clientId & 0xff;
    ctx.buf[ctx.pos++] = 0x00;
    ctx.buf[ctx.pos++] = 0x04;

    // Append attribute value.
    idHex.copy(ctx.buf, ctx.pos);
    ctx.pos += 20;
};
    ...
    for (i = 0; i < this._attrs.length; ++i) {
        ...
        switch (code) {
            ...
            case 0x0032: // timestamp
                ...
                break;
            case 0x0033: // clientId
                Ctor._writeClientId(ctx, this._attrs[i].value);
                break;
    ...
    while (ctx.pos < ctx.buf.length) {
        ...
        switch (code) {
            ...
            case 0x0032: // xorMappedAddr
                ...
                break;
            case 0x0033: // clientId
                attrVal = Ctor._readClientId(ctx);
                break;
            case 0x0006: // username
            case 0x0007: // password
            case 0x0008: // msgIntegrity
