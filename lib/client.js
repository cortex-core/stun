...
Client.prototype._discover = function () {
    ...
    this._soc0.bind(0, this._local.addr, function () {
        ...
        */
        ...
        self._breq0.addAttribute('clientId', self._id);
    ...
    if (this._elapsed >= Math.min((1 << this._retrans), 16)) {
        ...
        if (this._state == State.NBDaDp ||
            ...
            if (this._ef.ad != undefined && this._ef.pd != undefined) {
                if (this._retrans < 9) {
                    var regBind = new Message();
                    regBind.init();
                    regBind.setType('regB');
                    regBind.addAttribute('mappedAddr', this.getMappedAddr());
                    regBind.addAttribute('clientId', this._id);
                    regBind.setTransactionId(Ctor._randTransId());
                    sbuf = regBind.serialize();
                    this._soc1.send(sbuf, 0, sbuf.length, this._port0, this._serv0);
                    console.log("EF-Rtx1: retrans=" + this._retrans + " elapsed=" + this._elapsed);
                } else {
                    clearInterval(this._intervalId);
    ...
    if (this._state == State.NBDaDp) {
        ...
        */
        ...
        if (this._ef.ad !== undefined && this._ef.pd !== undefined) {
            ...
            var mappedAddr = {
                'family': 'ipv4',
                'port': this.getMappedAddr().port,
                'addr': this.getMappedAddr().address
            };
            regBind.addAttribute('mappedAddr', mappedAddr);
            regBind.addAttribute('clientId', this._id);
            regBind.setTransactionId(Ctor._randTransId());
            sbuf = regBind.serialize();
            this._soc1.send(sbuf, 0, sbuf.length, this._port0, this._serv0);
