...
Server.prototype._onReceived = function (sid, msg, rinfo) {
    ...
    // Check if it has id
    var id = stunmsg.getAttribute('clientId');
    if (id != undefined) {
        console.log(id);
        this._registeredClients[id] = {
            'family': 'ipv4',
            'port': rinfo.port,
            'addr': rinfo.address
        };
...
/**
 * Returns mapped address details of client.
 */
Server.prototype.getClientRoute = function (id) {
    return this._registeredClients[id];
};
